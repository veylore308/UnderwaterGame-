...
-- Phase 3: surface fishing mode (boat)
FishingController._surfaceMode = false
FishingController._cachedEquippedSurfaceRod = "CastingRod"
FishingController._weatherState = "Calm"
FishingController._boatHandler = nil
...
function FishingController:KnitStart()
    print("[FishingController] Started (Phase 2/3)")

    self._rodHandler = FishingRodHandler.new()
    self._fishingHUD = FishingHUD.new()

    -- Phase 3: boat controls + surface HUD
    local BoatHandler = require(script.Parent.Parent:WaitForChild("Handlers"):WaitForChild("BoatHandler"))
    self._boatHandler = BoatHandler.new(self.Services, self)
...
function FishingController:KnitInit()
    ...
    -- Phase 3: track weather for surface modifier display
    local weatherService = self.Services.WeatherService
    if weatherService then
        weatherService.Client.WeatherChanged:Connect(function(data)
            if data and data.State then
                self._weatherState = data.State
            end
        end)
    end
end
...
-- ============================================================
-- Phase 3: Surface mode API (called by BoatHandler)
-- ============================================================
function FishingController:SetSurfaceMode(enabled)
    self._surfaceMode = enabled and true or false
    if not self._surfaceMode and self._phase ~= "Idle" then
        self:_endFishing() -- can't fish underwater with a surface cast
    end
end

function FishingController:IsSurfaceMode()
    return self._surfaceMode
end

--- Server bobber drift update (surface) — move the visual bobber
function FishingController:OnBobberDrifted(castId, position)
    if self._castId ~= castId then return end
    if self._rodHandler then
        self._rodHandler:MoveBobberTo(position)
    end
end
...
function FishingController:_wirePlayerDataService()
    ...
    local function onPlayerDataReceived(data)
        if data and data.Gear then
            ...
            -- Phase 3: cache the surface rod track
            if data.Gear.EquippedSurfaceRod then
                self._cachedEquippedSurfaceRod = data.Gear.EquippedSurfaceRod
            end
        end
    end
