...
local SurfaceFishAI = require(script.Parent:WaitForChild("SurfaceFishAI")) -- Phase 3
...
function FishSpawner:SpawnFish(species)
    ...
    -- Create the NPC (Surface zone uses the lightweight SurfaceFishAI; others FishNPC)
    local waypoints = self:GetWaypointsForSpecies(species)
    local fish
    if self.ZoneKey == "Surface" then
        fish = SurfaceFishAI.new(species, spawnPos, {
            Waypoints = waypoints,
            Zone = self.ZoneKey,
        })
    else
        fish = FishNPC.new(species, spawnPos, {
            Waypoints = waypoints,
            Zone = self.ZoneKey,
        })
    end
...
function FishSpawner:SpawnSchool(species, leaderFish)
    ...
    for i = 2, schoolSize do
        ...
        local followerFish
        if self.ZoneKey == "Surface" then
            followerFish = SurfaceFishAI.new(species, followerPos, {
                Waypoints = self:GetWaypointsForSpecies(species),
                Zone = self.ZoneKey,
            })
        else
            followerFish = FishNPC.new(species, followerPos, {
                Waypoints = self:GetWaypointsForSpecies(species),
                Zone = self.ZoneKey,
            })
        end
...
-- ============================================================
-- Respawn cycle: fill population to cap (zone-aware, weather-aware)
-- Phase 3: weather spawn-table modifiers, rain spawn boost, storm
-- rare-only weighting, Storm Marlin weather-lock, Moonfish dawn/dusk
-- gate, predator-near-school spawning.
-- ============================================================
function FishSpawner:RunRespawnCycle(luckBonus, weatherState, weatherMods)
    luckBonus = luckBonus or 0
    weatherState = weatherState or "Calm"
    weatherMods = weatherMods or { Common = 1, Uncommon = 1, Rare = 1, Legendary = 1 }
    local maxPop = self.ZoneConfig.Population.MaxConcurrentFish
    local deficit = maxPop - self._population
    ...
    -- Weather-adjusted weights (renormalized)
    local weatherEntries = {}
    local weatherTotal = 0
    for _, entry in ipairs(spawnTable) do
        local sp = entry.Species
        local mod = weatherMods[sp.Rarity] or 1

        -- Storm Marlin: weather-locked — 0% in calm (GDD 4.6.2)
        if sp.Key == "StormMarlin" and weatherState == "Calm" then
            mod = 0
        end
        -- Moonfish: dawn/dusk window only (GDD 4.6.4.7)
        if sp.Key == "Moonfish" and not self:_isDawnDuskWindow() then
            mod = 0
        end

        if mod > 0 then
            weatherTotal = weatherTotal + entry.CumulativeWeight * mod
            table.insert(weatherEntries, { Species = sp, Weight = entry.CumulativeWeight * mod })
        end
    end

    if weatherTotal <= 0 then return end

    -- Rain: +20% fish spawn rate (task brief)
    local spawnCount = math.min(deficit, 3)
    if weatherState == "Rain" then
        spawnCount = math.min(deficit, math.ceil(spawnCount * 1.2))
    end

    for i = 1, spawnCount do
        local roll = math.random() * weatherTotal
        local selectedSpecies = nil
        for _, entry in ipairs(weatherEntries) do
        ...
        if selectedSpecies then
            ...
            elseif selectedSpecies.Schooling then
                ...
                -- Predators spawn near existing schools (GDD 4.6.3)
                if self._isPredator(selectedSpecies) then
                    self:SpawnPredatorNearSchool(selectedSpecies)
                else
                    self:SpawnFish(selectedSpecies)
                end
            end
...
-- ============================================================
-- Phase 3: predator-near-school spawning (Bonito, Sailfish, Marlin)
-- ============================================================
function FishSpawner:_isPredator(species)
    return species.Key == "Bonito"
        or species.Key == "Sailfish"
        or species.Key == "StormMarlin"
end

function FishSpawner:SpawnPredatorNearSchool(species)
    -- Find an existing school leader to anchor the predator near
    local anchor = nil
    for _, fish in pairs(self._fish) do
        if fish.Species and fish.Species.Schooling and not fish:IsHooked() then
            anchor = fish:GetPosition()
            break
        end
    end
    if not anchor then
        return self:SpawnFish(species)
    end
    local angle = math.random() * math.pi * 2
    local pos = anchor + Vector3.new(math.cos(angle) * 12, 0, math.sin(angle) * 12)
    local fish = self:SpawnFishAt(species, pos)
    return fish
end

function FishSpawner:SpawnFishAt(species, position)
    if self._population >= (self.ZoneConfig.Population.MaxConcurrentFish or 25) then return nil end
    if species.Rarity == "Legendary" and self._legendaryCount >= self._legendaryCap then return nil end

    local fish
    if self.ZoneKey == "Surface" then
        fish = SurfaceFishAI.new(species, position, { Waypoints = {}, Zone = self.ZoneKey })
    else
        fish = FishNPC.new(species, position, { Waypoints = {}, Zone = self.ZoneKey })
    end
    fish:SetZone(self.ZoneKey)
    local model = self:_createFishModel(species, position)
    fish:Initialize(model, self)
    self._fish[fish.Id] = fish
    self._population = self._population + 1
    if species.Rarity == "Legendary" then
...
-- Phase 3: dawn/dusk window (first/last 3 min of the 20-min day cycle,
-- same window as the Spectral Ray — GDD 4.6.4.7)
function FishSpawner:_isDawnDuskWindow()
    local clockTime = Lighting.ClockTime
    return (clockTime >= 4.5 and clockTime < 8) or (clockTime >= 16 and clockTime < 19.5)
end
...
function FishSpawner:_getSpawnPosition(species)
    -- Phase 3: Surface fish spawn near players (interest-based population)
    if self.ZoneKey == "Surface" then
        return self:_getSurfaceSpawnPosition(species)
    end
...
-- ============================================================
-- Phase 3: Surface spawn position — 40-100 studs from a random
-- player (2D), 0-8m below the waterline (GDD 4.6.3)
-- ============================================================
function FishSpawner:_getSurfaceSpawnPosition(species)
    local players = Players:GetPlayers()
    if #players == 0 then
        return Vector3.new(math.random(-800, 800), -3, math.random(-800, 800))
    end
    ...
    local y = -1 - math.random() * 7 -- 0-8m below surface

    return Vector3.new(
        math.clamp(origin.X + math.cos(angle) * dist, -1400, 1400),
        y,
        math.clamp(origin.Z + math.sin(angle) * dist, -1400, 1400)
    )
end
...
function FishSpawner:Update(dt)
    ...
    for fishId, fish in pairs(self._fish) do
        ...
        -- Phase 3: Surface fish despawn beyond 150 studs of ANY player
        -- (population is per-active-area, not per-map — GDD 4.6.3)
        if self.ZoneKey == "Surface" then
            local nearest = self:_nearestPlayerDistance(fish:GetPosition())
            if nearest > (self.ZoneConfig.Population.DespawnBoundary or 150) then
                table.insert(toRemove, fishId)
            end
        else
...
-- Phase 3: nearest player distance (2D for surface fish)
function FishSpawner:_nearestPlayerDistance(position)
