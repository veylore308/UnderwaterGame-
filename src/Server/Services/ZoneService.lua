...
function ZoneService:KnitStart()
    ...
    -- Phase 3: Surface zone (shared ocean, weather-driven spawns)
    local surfaceConfig = Shared.Constants.ZoneConfigs.Surface
    if surfaceConfig then
        local spawner = FishSpawner.new("Surface", surfaceConfig, Knit)
        self._zones["Surface"] = {
        ...
        self._spawners["Surface"] = spawner
        print("[ZoneService] Initialized Surface zone with weather-driven FishSpawner")
    end
...
function ZoneService:_respawnFish(zoneKey, zone, config)
    ...
    -- Phase 3: pass current weather state + spawn modifiers (Surface spawns
    -- re-roll per weather: Calm normal, Rain +20% rate, Storm rare-heavy)
    local weatherState = "Calm"
    local weatherMods = nil
    if self.Services.WeatherService then
        weatherState = self.Services.WeatherService:GetState()
        weatherMods = self.Services.WeatherService:GetSurfaceSpawnMods()
    end

    -- Run the spawner's respawn cycle
    spawner:RunRespawnCycle(luckBonus, weatherState, weatherMods)
...
-- ============================================================
-- Phase 3: Weather → spawner hook (called by WeatherService on roll)
-- ============================================================
function ZoneService:SetWeatherForZones(state, spawnMods)
    -- Stored for the next respawn cycle; the surface spawner reads it
    -- via _respawnFish on each cycle (45s). Also nudge an immediate
    -- respawn so weather changes are felt quickly.
    self._weatherState = state
    self._weatherMods = spawnMods
    local surfaceZone = self._zones["Surface"]
    if surfaceZone then
        surfaceZone.LastRespawnTime = 0 -- force respawn on next cycle
    end
end
...
function ZoneService:_getMaxLuckBonus(zoneKey)
    ...
    for player, zKey in pairs(self._playerZones) do
        if zKey == zoneKey then
            local data = self.Services.PlayerDataService:GetData(player)
            if data then
                -- Phase 3: surface zone uses surface rod luck (GDD 4.6.3)
                if zoneKey == "Surface" then
                    local sRod = data.Gear.EquippedSurfaceRod
                    local sStats = Shared.Constants.SurfaceRodTiers.GetByKey(sRod)
                    if sStats then
                        maxLuck = math.max(maxLuck, sStats.LuckBonus or 0)
                    end
                else
